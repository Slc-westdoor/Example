...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
         ...
         ***配置定时器，自动滚动广告栏
         */
        _timer = [NSTimer scheduledTimerWithTimeInterval:3.0f target:self selector:@selector(timerAction:) userInfo:nil repeats:YES];
...
- (void)setArray:(NSArray *)imgArray{
    ...
    if (_totalNum>0) {
        ...
        _imageNum.text = [NSString stringWithFormat:@"%ld / %ld",(long)_pageControl.currentPage+1,(long)_totalNum];
